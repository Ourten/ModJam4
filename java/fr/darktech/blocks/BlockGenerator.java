...
public class BlockGenerator extends BlockContainer {
	...
	public void onBlockAdded(World w, int x, int y, int z) {
		...
		if (tile instanceof TileEntityGenerator) {
			((TileEntityGenerator) tile).setNetwork(NetworkManager.getNetworkManager().registerTileEntityInNetwork((INetCompatible) tile, "network_" + System.currentTimeMillis()));
		}
	...
	public void breakBlock(World w, int x, int y, int z, Block b, int p_149749_6_) {
		TileEntity tile = w.getTileEntity(x, y, z);
		if (tile instanceof TileEntityGenerator)
			NetworkManager.getNetworkManager().removeTileEntity((INetCompatible) tile, ((TileEntityGenerator) tile).getNetwork().mapName);
	}
